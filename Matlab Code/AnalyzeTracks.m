function CalculationsStruct = AnalyzeTracks(AllTracks, Parameters)
...
CalculationsStruct = table(ExperimentName, TiffFileName, CellName,...
  isDivided,isDaughter,nImages);
CalculationsStruct = [CalculationsStruct, struct2table(R)];
% Create description:
distUnit = Parameters.PixelSize_Unit;
timeUnit = Parameters.TimeInterval_Unit;
CalculationsStruct.Properties.VariableDescriptions ={...
  '{Experiment Name}',...
  '{Tiff File Name}',...
  '{Cell Name}',...
  '{isDivided} ->> True if cell divided during measurement  ',...
  '{isDaughter} ->> True/False  ',...
  '{nImages} ->> Number of Analyzed Images  ',...
  ['{Distance} ->> Total distance traveled [' distUnit ']  '],...
  ['{Displacement} ->> distance between End Point and Start Point [' distUnit ']  '] ,...
  ['{TrajectoryTime} ->> Total trajectory time [' timeUnit ']  '] ,...
  '{Directionality} ->> ratio: Displacement/TotalDistance  ',....
